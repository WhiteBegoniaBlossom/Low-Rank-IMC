...
Copied from tyler-main/ITLP/model/dgl/layers.py
"""
import torch
...
class RGCNLayer(nn.Module):
    def __init__(self, inp_dim, out_dim, aggregator, bias=None, activation=None,
                 dropout=0.0, edge_dropout=0.0, is_input_layer=False):
...
class RGCNBasisLayer(RGCNLayer):
    def __init__(self, inp_dim, out_dim, aggregator, attn_rel_emb_dim,
                 num_rels, num_bases=-1, bias=None, activation=None,
                 dropout=0.0, edge_dropout=0.0, is_input_layer=False,
                 has_attn=False, is_comp=''):
        super(
            RGCNBasisLayer,
            self).__init__(
            inp_dim,
    ...
    def propagate(self, g, attn_rel_emb=None):
        ...
        def comp(h, edge_data):
            ...
            if self.is_comp == 'mult':
                ...
                raise KeyError(f'composition operator {self.is_comp} not recognized.')
        ...
        # Ensure 'h' exists for nodes without incoming edges
        if 'h' not in g.ndata:
